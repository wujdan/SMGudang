            ...
            AfterSheet::class => function (AfterSheet $event) use ($dari, $sampai, $statusLabel, $search, $pekerjaan, $lastCol) {
                ...
                foreach ($pekerjaan as $p) {
                    ...
                    $sheet->getStyle("A{$row}:{$lastCol}{$row}")->applyFromArray([
                        ...
                        'borders' => [
                            'allBorders' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['argb' => 'FF9DB8CC']],
                        ],
                    ...
                    $headers = ['No', 'Barang', 'Kategori', 'Jumlah', 'Satuan', 'Tgl Keluar', 'Status Barang'];
                    ...
                    if ($p->transaksi->isEmpty()) {
                        ...
                        foreach ($p->transaksi as $t) {
                            $tglKeluar = $t->tanggal_keluar ? date('d-m-Y', strtotime($t->tanggal_keluar)) : '-';
                            $statusBarang = $t->status_pinjam ? $t->status_label : 'Keluar Permanen';
                            if ($t->status_pinjam && $t->tgl_kembali_aktual) {
                                $statusBarang .= ' (' . date('d-m-Y', strtotime($t->tgl_kembali_aktual)) . ')';
                            }

                            $sheet->setCellValue("A{$row}", $no);
                            $sheet->setCellValue("B{$row}", $t->barang->nama_barang ?? '-');
                            $sheet->setCellValue("C{$row}", $this->formatKategori($t->barang->kategori ?? null));
                            $sheet->setCellValue("D{$row}", $t->jumlah);
                            $sheet->setCellValue("E{$row}", $t->barang->satuan ?? '-');
                            $sheet->setCellValue("F{$row}", $tglKeluar);
                ...
                // Tinggi baris header pekerjaan sedikit lebih tinggi
